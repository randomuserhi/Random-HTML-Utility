(function() {
    ...
    RHU.module(new Error(), "rhu/theme",
        ...
        function({ types: { ThemeVariable }, Style }) {
            const Theme = function<T extends {} = {}>(factory: (worker: RHU.Theme.Factory) => T): RHU.Style.ClassName<T> {
                const cn = new Style.cn<T>();
                let generatedCode = `.${cn} {`;
                const generator = function<T extends {} = {}>(first: TemplateStringsArray, ...interpolations: (string | RHU.Theme.ThemeVariable)[]): RHU.Theme.ThemeVariable<T> {
    ...
    RHU.module(new Error(), "rhu/style", 
        ...
        function({ style: { cn }, Theme }) {
            ...
            const css = function (style: RHU.Style.StyleDeclaration): string {
                let result = "";
            ...
            const Style = function<T>(factory: (worker: RHU.Style.Factory) => T): T {
                let generatedCode = "";
                const generator = function (first: TemplateStringsArray, ...interpolations: (string | RHU.Style.ClassName | RHU.Style.StyleDeclaration)[]): void {
